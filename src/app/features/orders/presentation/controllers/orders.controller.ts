...
if(req.user.roles.includes("Buyer")){
  ...
  if(mappedSendedproducts.length > 0 && mappedSendedproducts.length < mappedProducts.length && ordersUpsertRequest.status === OrderStatusEnum.PARTSENDING){
...
//console.log(ordersUpsertRequest,"ordersUpsertRequest")
    ...
    const result =
      await this
        .commandBus
        .execute<OrdersUpsertCommand, AppResult<OrdersGetResult>>(
          command,
        );
  ...
  public async changeStatus(
    ...
    const result =
      await this
        .commandBus
        .execute<OrdersChangeStatusCommand, AppResult<OrdersGetResult>>(
          command,
        );
  ...
  public async delete(
    ...
    const command = new OrdersDeleteCommand(
        ordersDeleteRequest.id,
      userId,
    );
    ...
    const result =
      await this
        .commandBus
        .execute<OrdersDeleteCommand, AppResult<null>>(
          command,
        );
  ...
  public async get(
    ...
    try {
      ...
      const orderResult = await this.queryBus.execute<OrdersGetQuery, AppResult<OrdersGetResult>>(orderQuery);
      ...
      const tenderResult = await this.queryBus.execute<TendersGetQuery, AppResult<TendersGetResult>>(tenderQuery);
  
      if (tenderResult.isSuccess) {
        orderData = { ...orderData, Tender: tenderResult.data };
        const BuycompanyQuery = new CompaniesGetQuery(tenderResult.data.companyId);
        const BuycompanyResult = await this.queryBus.execute<CompaniesGetQuery, AppResult<Company>>(BuycompanyQuery);
      ...
      const companyResult = await this.queryBus.execute<CompaniesGetQuery, AppResult<Company>>(companyQuery);
  ...
  const productCompanyPromises = orderData.products.map(async (product) => {
    ...
    const productCompanyResult = await this.queryBus.execute<CompaniesGetQuery, AppResult<Company>>(productCompanyQuery);
    return {
      ...product,
  ...
  orderData = { ...orderData, products: updatedProducts };
   ...
   const SendedproductsCompanyPromises = orderData.Sendedproducts.map(async (product) => {
    const SendedproductsCompanyQuery = new CompaniesGetQuery(product.companyId);
    const SendedproductsCompanyResult = await this.queryBus.execute<CompaniesGetQuery, AppResult<Company>>(SendedproductsCompanyQuery);
    return {
      ...product,
  ...
  public async getAll(
    ...
    const result = await this.queryBus.execute<OrdersGetAllQuery, AppResult<Array<OrdersGetAllResult>>>(
      query,
    );
    ...
    return AppResponse
      .create<Array<OrdersGetAllResponse>>(
        result.isSuccess,
        result.key,
        result.message,
        responseData,
  ...
  public async getMyAll(
    ...
    const responseData = result.data ? result.data.filter(item=>item.userId === userId).map(
      order =>
  ...
  public async getAllTotal(
    ...
    const openedOrders = await this.queryBus.execute<OrdersGetAllQuery, AppResult<Array<OrdersGetAllResult>>>(
      query
    );
  ...
  public async getAllTotalByStatus(
    ...
    const Orders = await this.queryBus.execute<OrdersGetAllQuery, AppResult<Array<OrdersGetAllResult>>>(
      query
    );
  ...
  private async createPDFInvoice(order,company,  lang: 'en' | 'ar'): Promise<string> {
    ...
    private async createAndUploadPDF(order, lang: 'en' | 'ar'): Promise<string> {
      ...
      const s3Url = await S3UploadService.uploadFile(file as Express.Multer.File, folderPath);
