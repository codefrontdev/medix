...
import { AdminCompaniesUpsertRequest } from "../contracts/request/admin-companies-upsert.request";
...
export class CompaniesController {
  ...
  public async upsert(
    @UploadedFiles()
    files: {
      authorizationFileUrl?: Express.Multer.File[];
      registeringFileUrl?: Express.Multer.File[];
      logoMedia?: Express.Multer.File[];
    },
    @Body() companiesUpsertRequest: CompaniesUpsertRequest,
    @Req() req: any
    ...
    if (files.authorizationFileUrl && files.authorizationFileUrl[0]) {
      authorizationFileUrl = await S3UploadService.uploadFile(
        files.authorizationFileUrl[0],
        folderPath
      );
    }
    if (files.registeringFileUrl && files.registeringFileUrl[0]) {
      registeringFileUrl = await S3UploadService.uploadFile(
        files.registeringFileUrl[0],
        folderPath
      );
    }
    if (files.logoMedia && files.logoMedia[0]) {
      logoMediaUrl = await S3UploadService.uploadFile(
        files.logoMedia[0],
        folderPath
      );
    }
    ...
    const command = new CompaniesUpsertCommand(
      ...
      companiesUpsertRequest.stampedAuthorizationFormUrl,
      ...
      companiesUpsertRequest.OpportunityNr,
      companiesUpsertRequest.TenderNr
    );

    const result = await this.commandBus.execute<
      CompaniesUpsertCommand,
      AppResult<CompaniesGetResult>
    >(command);
    ...
    const response = AppResponse.create<CompaniesGetResponse>(
      result.isSuccess,
      result.key,
      result.message,
      result.data,
  ...
  public async Adminupsert(
    @Body() companiesUpsertRequest: AdminCompaniesUpsertRequest,
    @Req() req: any
  ): Promise<AppResponse<CompaniesGetResponse>> {
    const { userId, roles } = req.user;
    ...
    //insert into user-company
    await this.companiesService.addUserToCompany(
      createObjectId(result.data.userId),
      createObjectId(result.data.id),
      companiesUpsertRequest.ownerType
    );
    const response = AppResponse.create<CompaniesGetResponse>(
      result.isSuccess,
  ...
  public async delete(
    ...
    const command = new CompaniesDeleteCommand(
      companiesDeleteRequest.id,
      userId
    );

    const result = await this.commandBus.execute<
      CompaniesDeleteCommand,
      AppResult<null>
    >(command);
  ...
  public async get(
    ...
    const query = new CompaniesGetQuery(companiesGetRequest.id);
  ...
  public async getAll(
    ...
    const { userId, roles } = req.user;
    const query = new CompaniesGetAllQuery(
    ...
    const result = await this.queryBus.execute<
      ...
      AppResult<Array<CompaniesGetAllResponse>>
    >(query);

    const response = AppResponse.create<Array<CompaniesGetAllResponse>>(
      result.isSuccess,
      result.key,
      result.message,
      result.data,
  ...
  public async getAllUsers(
    ...
    const users =
      await this.userCompaniesService.getAllUsersByCompanyId(ComapnyId);
  ...
  public async getAllAttchments(
    ...
    // Create the response
    const response = AppResponse.create<
      Array<CompaniesGetAllAttachmentsResponse>
    >(
      true, // Assuming the operation is successful
      null,
  ...
  public async getmy(
    ...
    const query = new CompaniesGetMineQuery(userId);

    const result = await this.queryBus.execute<
      CompaniesGetMineQuery,
      AppResult<CompaniesGetResult>
    >(query);
