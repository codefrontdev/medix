  ...
  implements ICommandHandler<AuthLoginCommand, AppResult<AuthDataResult>>
{
  ...
  public async execute(
    ...
    if (!foundUser.isEmailConfirmed) {
      throw AppResult.createError(AuthError.emailNotConfirmed );
    }
    ...
    if (!isPasswordCorrect) {
      throw AppResult.createError(AuthError.passwordIncorrect);
    }

    const userToken = await this.userTokenFactory.save(foundUser._id);

    const resultData = AuthDataResult.create(
      UsersInfoResult.create(
        foundUser._id,
        foundUser.nickName,
        foundUser.email,
      ...
      AuthTokensResult.create(userToken.accessToken, userToken.refreshToken)
    );

    return AppResult.createSuccess<AuthDataResult>(null, null, resultData);
  }
}
