  ...
  implements ICommandHandler<AuthsUpsertCommand, AppResult<UsersGetResult>>
{
  ...
  public async execute(
    ...
    if (isInsert) {
      ...
      if (foundEntity !== null) {
        throw AppResult.createError(UsersError.emailTaken);
      }

      if (command.phoneNumber !== "") {
        foundEntity = await this.usersRepository.getByPhoneNumber(
          command.phoneNumber
        );

        if (foundEntity !== null) {
          throw AppResult.createError(UsersError.phoneNumberTaken);
        }
      }
    }

    var entity = await this.authFactory.save(
      command.id,
      command.nickName,
      command.email,
      command.phoneNumber,
