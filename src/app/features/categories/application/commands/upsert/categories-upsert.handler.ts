...
export class CategoriesUpsertHandler
  implements
    ICommandHandler<CategoriesUpsertCommand, AppResult<CategoriesGetResult>>
{
  public constructor(private readonly categoryFactory: CategoryFactory) {}

  public async execute(
    command: CategoriesUpsertCommand
  ): Promise<AppResult<CategoriesGetResult>> {
    const entity = await this.categoryFactory.save(
      command.id,
      command.name,
      command.TagName,
      command.parentId,
