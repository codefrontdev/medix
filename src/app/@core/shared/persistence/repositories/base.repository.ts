...
> {
  ...
  public async get(
    ...
    return this.schemaFactory.createFromSchema(entityDocument as TSchema);
  }
  ...
  public async getAll(
    ...
    return entityDocuments.map((entityDocument) =>
      this.schemaFactory.createFromSchema(entityDocument as TSchema)
    );
  }
  ...
  public async getAllAsResult(
    ...
    const totalRecords =
      !withPaging ? 0 : await this.model.countDocuments(filterQuery);

    const appPaging =
      !withPaging ? null : (
        AppPaging.calc(withPaging, totalRecords, pageSize, pageNumber)
      );

    const entityDocuments = await query.exec();

    const entities = entityDocuments.map((entityDocument) =>
      this.schemaFactory.createFromSchema(entityDocument as TSchema)
    );

    return AppResult.createSuccess(null, null, entities, appPaging);
  }
  ...
  public async insertAll(entities: TEntity[]): Promise<void> {
  ...
  public async getAndUpdate(
    ...
    const updatedEntityDocument = await this.model.findOneAndReplace(
      filterQuery,
      schema,
      {
        new: true,
  ...
  public async getAndDelete(
    ...
    const deletedEntityDocument =
      await this.model.findOneAndDelete(filterQuery);
  ...
  public async delete(filterQuery: FilterQuery<TSchema>): Promise<boolean> {
    const deleteResult = await this.model.deleteOne(filterQuery);

    return deleteResult.deletedCount >= 1;
  }

  public async deleteAll(filterQuery: FilterQuery<TSchema>): Promise<boolean> {
  ...
  private hasDates(object: any): boolean {
    const hasCreatedAt =
      object != null && typeof object.createdAt !== "undefined";
    const hasUpdatedAt =
      object != null && typeof object.updatedAt !== "undefined";
    const hasDeletedAt =
      object != null && typeof object.deletedAt !== "undefined";
  ...
  private orderBy(
    ...
    if (orderByCriteria != null && orderByCriteria.length != 0) {
      const sortCriteria = orderByCriteria.reduce(
        (acc, { field, direction }) => {
          acc[field] = direction === OrderDirectionEnum.ASC ? 1 : -1;
      ...
      query = query.sort(sortCriteria as { [key: string]: SortOrder });
    }
