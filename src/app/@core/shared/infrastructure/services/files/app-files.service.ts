...
import { existsSync, mkdirSync, unlinkSync, move, moveSync, copySync } from 'fs-extra';
import { Media } from "src/app/features/medias/domain/entities/media";
import { getFileExtension, mediasConstants } from "src/app/features/medias/domain/constants/medias-constants";
...
export class AppFilesService {
  ...
  public async moveMultipleFiles(
    ...
    const combinedDestDirectoryPath =
      join(
        rootPath,
        directoryPath,
      );
    ...
    for (const media of medias) {
      ...
      const combinedSrcFilePath =
        join(
          rootPath,
          media.url,
        );

      const combinedDestFilePath =
        join(
  ...
  public async deleteMultipleFiles(
    ...
    for (const media of medias) {
      ...
      try {
        ...
        media.url =
          join(
            mediasConstants.paths.recyclerBin,
            media.uniqueName,
          );
        ...
        console
          .error(
            `Failed to move file ${media.uniqueName}: ${error}`,
          );
      }
    }
  ...
  public createAppDirectories(): void {
    this
      .createDirectories(
        false,
        ...mediasConstants.directories,
      );

    this
      .createDirectories(
        true,
        ...mediasConstants.protectedDirectories,
      );
  }
  ...
  private getRootPath(
    ...
    return isProtected ?
      join(
        rootPath,
        mediasConstants.paths.uploadsProtected,
      )
      :
      join(
        rootPath,
