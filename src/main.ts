...
async function bootstrap(): Promise<void> {
  ...
  app.useStaticAssets(getPublicDirectory(), {
...
async function pipes(app: NestExpressApplication): Promise<void> {
...
async function filters(
  ...
  console.log("first")
  app.useGlobalFilters(new AppGlobalExceptionFilter(appLoggerService));
}
...
async function middlewares(
  ...
  app.use((req, res, next) =>{
    console.log("req#######################")
    new RequestsLoggerMiddleware(appLoggerService).use(req, res, next)
  });

  const jwtProviderService = app.get(JwtProviderService);

  app.use((req, res, next) =>
    new ProtectedFilesMiddleware(jwtProviderService).use(req, res, next)
  );
}
...
async function run(
  ...
  const appConfigsService = app.get(AppConfigsService);
...
async function hotReload(app: NestExpressApplication): Promise<void> {
  if (module.hot) {
    module.hot.accept();
